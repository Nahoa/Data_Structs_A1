...
public:
	...
	Array(const Array & copy):m_array(nullptr),m_length(0),m_start_index(0)//Copy Ctor
	{
	...
	T & operator[](int index) //overloads the template [] operator 
	{
		...
		if (index >= m_length) //checks to see if the thing being checked is in the length range
		{
